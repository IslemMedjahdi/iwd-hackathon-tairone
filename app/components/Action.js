...
export default function Action({ image, name, color, desc, textColor }) {
    ...
    return (
        <View  style={{ marginBottom: 20, }}>
            <View  style={{ borderTopRightRadius: 15, borderTopLeftRadius: 15, borderBottomRightRadius: open ? 0 : 15, borderBottomLeftRadius: open ? 0 : 15, overflow: "hidden", backgroundColor: color + "90" }}>
                <Pressable onPress={() => setOpen(!open)}  android_ripple={{ color: color }} style={{ padding: 20, justifyContent: "space-between", alignItems: "center", flexDirection: "row", }}>
                    <Image source={image} />
                    <Text style={{ color: textColor, fontSize: 20, fontWeight: "bold" }}>{name}</Text>
                    <Pressable onPress={() => setOpen(!open)}  android_ripple={{ color: "#00000020", borderless: true, radius: 25 }}>
            ...
            </View>
            {
                ...
                <View style={{ backgroundColor: color + "90", padding: 20, borderBottomRightRadius: 15, alignItems: "flex-end", borderBottomLeftRadius: 15 }}>
                    <Text style={{ color: textColor,fontSize: 16,alignSelf: "baseline" }}>{desc}</Text>
